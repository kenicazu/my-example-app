...
resource "aws_cloudwatch_event_rule" "codecommit_event_rule" {
  name_prefix   = "${local.pipeline_name}-rule"
  description   = "Detect commits to ${var.repository_name} CodeCommit repo on branch ${var.branch_name}"
...
resource "aws_iam_role" "pipeline_trigger_role" {
  name_prefix = "${var.repository_name}-trigger-role"
...
data "aws_iam_policy_document" "pipeline_trigger_role_policy_document" {
  #checkov:skip=CKV_AWS_109: Permissions needed
  statement {
    actions = [
      "iam:PassRole"
    ]
...
resource "aws_iam_policy" "pipeline_trigger_role_policy" {
  name_prefix = "${local.pipeline_name}-trigger-role-policy"
  policy      = data.aws_iam_policy_document.pipeline_trigger_role_policy_document.json
}
resource "aws_iam_role_policy_attachment" "pipeline_trigger_role_policy_attachment" {
  policy_arn = aws_iam_policy.pipeline_trigger_role_policy.arn
  role       = aws_iam_role.pipeline_trigger_role.name
}

resource "aws_cloudwatch_event_target" "pipeline_trigger_event" {
  target_id = "${local.pipeline_name}-trigger-event"
  rule      = aws_cloudwatch_event_rule.codecommit_event_rule.name
  arn       = local.pipeline_arn
  role_arn  = aws_iam_role.pipeline_trigger_role.arn
}
